...
function findLongestWordLength4(str) {
  ...
  if (words.length == 1) {  // words only has 1 element left that is the longest element
    return words[0].length;
  }

  return Math.max(   // if words has multiple elements, remove the first element and recursively call the function
    words[0].length,
